	...
	void patchtype(string type, vector<string> var_types ){
		...
		for(itr=var_types.begin(); itr!=var_types.end(); itr++)
		{
			...
			if(search_var((*itr), active_func, NULL)!=1)
				...
				cout<<endl<<(*itr)<<": Variable Name with same identifier already exists!\n";
...
if: IF rlb op_rel rrb THEN clb {glob_level++;} stmt_list {printf("--------IF--------\n\n");symbolTable();
														vector<varEntry>::iterator itr;
														for(itr=active_func->variable.begin(); itr!=active_func->variable.end();itr++)
														{
															// cout<<"Variable "<<(*itr).name<<endl;
    ...
    | ELSE clb {glob_level++;} stmt_list{printf("--------ELSE--------\n\n");symbolTable();
														vector<varEntry>::iterator itr;
														for(itr=active_func->variable.begin(); itr!=active_func->variable.end();itr++)
														{
															cout<<"Variable "<<(*itr).name<<endl;
...
while: WHILE rlb op_rel rrb clb {glob_level++;} stmt_list {printf("-------WHILE---------\n\n");symbolTable();
														vector<varEntry>::iterator itr;
														for(itr=active_func->variable.begin(); itr!=active_func->variable.end();itr++)
														{
															cout<<"Variable "<<(*itr).name<<endl;
...
fn_call: ID {$$.node = new sattr;$1.node = new sattr;
			$$.node->name = strdup(yytext);
			funcCallTemp.name = $$.node->name; 
			$1.node->name = $$.node->name; 
			entry* funcPtr;
			
			if(searchFunction($$.node->name, &funcPtr)==0)
			{
			...
			if(searchFunction(yytext, &funcPtr)==1){
				call_func = funcPtr;
				cout<<call_func->name<<endl;
				$$.node->type = (*funcPtr).resultType;
				funcCallTemp.type = (*funcPtr).resultType;
			}

			} rlb fn_call_args rrb {entry* funcPtr;if(searchFunction(funcCallTemp.name, &funcPtr)){													
										if(call_func->parameter.size() != funcCallTemp.pno){
											printf("mismatch in number of parameters in declaration and call\n");
											$$.node->type = "errortype";
										}
										else
										{
											// cout<<(active_func->parameter).at(1).ele<<endl;fflush(stdout);
											// for(int i =0; i<funcCallTemp.pno; i++)
											// {
											// 	// cout<<funcCallTemp
											// 	if(funcCallTemp.var_types[i]==active_func->parameter[i].ele)
											// 		continue;
											// 	else
											// 	{
											// 		cout<<"Argument type mismatch in declaration and call: "<<funcCallTemp.var_types[i]<<" "<<active_func->parameter[i].ele<<endl;
											// 	}

											// }
											vector<string>::iterator itr;
											vector<varEntry>::iterator itr1;
											// for(itr=funcCallTemp.var_types.begin();itr!=funcCallTemp.var_types.end(); itr++)
											// 	cout<<"Type: "<<(*itr)<<endl;
											// for(itr1=call_func->parameter.begin();itr1!=call_func->parameter.end(); itr1++)
											// 	cout<<"Type1: "<<(*itr1).ele<<endl;
											for(itr=funcCallTemp.var_types.begin(), itr1 = call_func->parameter.begin();itr!=funcCallTemp.var_types.end(); itr++, itr1++)
											{
												if((*itr)==(*itr1).ele)
													continue;
												else
													cout<<"Argument type mismatch in declaration and call: "<<(*itr)<<" "<<(*itr1).ele<<endl;
											}
										}
										funcCallTemp.var_types.clear();
										funcCallTemp.pno = 0;
										funcCallTemp.name = "";
										funcCallTemp.type = "";
							} 
...
fn_call_args: /* empty */          {$$.node = new sattr;funcCallTemp.pno = 0;}                             
    | id_or_digits fn_call_args_prime                       {funcCallTemp.var_types.push_back($1.node->type);funcCallTemp.pno = 1 + funcCallTemp.pno;}
    | bool_prod fn_call_args_prime                          {funcCallTemp.pno = 1 + funcCallTemp.pno;
    														}
    ;
bool_prod: BOOL 											{$$.node->name = strdup(yytext);
															...
															$$.node->type = "bool";}
	;
fn_call_args_prime: /* empty */                     		 {$$.node = new sattr;}      
                    | comma fn_call_args 					{$$.node = new sattr;}

fn_def: /* empty */       {$$.node = new sattr;}                                      
    | datatype ID {$$.node = new sattr;$2.node = new sattr;$2.node->name=strdup(yytext);
    	entry* funcPtr;
		...
		{entry* funcPtr;funcTemp.name = $2.node->name;
			funcTemp.type = $1.node->type;
		if(searchFunction($2.node->name,&funcPtr)==0){
			// $$.node->type = $1.node->type; 
			// $$.node->param_types = $4.node->param_types;
			*($$.node) = funcTemp;
			enter_func($2.node->name,$1.node->type,&funcPtr);
			(*funcPtr).parameter=funcTemp.param_types;
		}
		active_func = funcPtr; glob_level=2;
		}
		clb stmt_list {	vector<varEntry>::iterator itr;printf("----------------\n\n");symbolTable();
							for(itr=active_func->variable.begin(); itr!=active_func->variable.end();itr++)
							{
								cout<<"Variable "<<(*itr).name<<endl;
								if((*itr).level == glob_level)
							...
							funcTemp.param_types.clear();
							funcTemp.name = "";
							funcTemp.type = "";

							glob_level--;}
							crb {searchFunction("main", &active_func);glob_level=0;} fn_def    
;
func_args: /* empty */       {$$.node = new sattr;}
    | datatype ID {$$.node = new sattr;$2.node = new sattr;$2.node->name=strdup(yytext);}
    						func_args_prime {
    				varEntry temp($2.node->name, "simple", "parameter", 1, $1.node->type);
    								// $$.node->param_types = $3.node->param_types;
    								// $$.node->param_types.push_back(temp); funcTemp.param_types = $$.node->param_types;
    								funcTemp.param_types.push_back(temp);
    								cout<<temp.name<<endl;
    							}
...
func_args_prime: /* empty */{$$.node = new sattr;}
                | {$$.node = new sattr;}comma func_args 
                ;
