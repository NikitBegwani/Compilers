...
prog:   fn_def  MAIN { sattr* temp = new sattr;
					...
					printf("Hi\n");fflush(stdout);
    					...
    					printf("\nI am in main \n");
    					$2.node = temp;
    			} rlb rrb clb  {glob_level++;} stmt_list crb {printf("----------------\n\n");symbolTable();}
...
if: IF rlb op_rel rrb THEN clb {glob_level++;} stmt_list {printf("----------------\n\n");symbolTable();
														vector<varEntry>::iterator itr;
    ...
    | ELSE clb {glob_level++;} stmt_list{printf("----------------\n\n");symbolTable();
														vector<varEntry>::iterator itr;
														for(itr=active_func->variable.begin(); itr!=active_func->variable.end();itr++)
														{
...
while: WHILE rlb op_rel rrb clb {glob_level++;} stmt_list {printf("----------------\n\n");symbolTable();
														vector<varEntry>::iterator itr;
														for(itr=active_func->variable.begin(); itr!=active_func->variable.end();itr++)
														{
															cout<<"Variable "<<(*itr).name<<endl;
